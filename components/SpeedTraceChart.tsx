...
const SpeedTraceChart: React.FC<SpeedTraceChartProps> = ({
  ...
  const loadSpeedData = async () => {
    ...
    try {
      ...
      const data = await fetchSpeedData(year, event, session, selectedDriver, lapNumber);
      setSpeedData(data);
    } catch (error) {
      console.error('Failed to fetch speed data:', error);
      Alert.alert('Error', 'Failed to load speed data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderChart = () => {
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#9ca3af', fontSize: 16, marginBottom: 16, textAlign: 'center' }}>
            Click load to view speed trace for selected driver
          </Text>
          <TouchableOpacity
            onPress={loadSpeedData}
            style={{
          ...
          >
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600' }}>
              {loading ? 'Loading...' : 'Load Speed Chart'}
            </Text>
          </TouchableOpacity>
        </View>
      );
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#f87171', fontSize: 16 }}>
            No speed data found for {selectedDriver} lap {selectedLap}
          </Text>
        </View>
      );
    }
    ...
    const chartData = {
      labels: speedData.filter((_, index) => index % Math.ceil(speedData.length / 6) === 0)
