...
const ThrottleChart: React.FC<ThrottleChartProps> = ({
  ...
  const toggleDriverSelection = (driverCode: string) => {
    setSelectedDrivers(prev => {
      ...
      if (shouldLoadChart && throttleData.length > 0) {
  ...
  const loadThrottleData = async () => {
    ...
    try {
      ...
      const data = await fetchThrottleData(year, event, session, selectedDrivers[0], lapNumber);
  ...
  const renderLoadButton = () => (
    <View style={{ alignItems: 'center', marginVertical: 20 }}>
      ...
      >
        ...
        <Text style={{ color: '#fff', fontWeight: '600', fontSize: 16 }}>Load Throttle Chart</Text>
      </TouchableOpacity>
      <Text style={{ 
  ...
  const renderChart = () => {
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#f87171', fontSize: 16 }}>
            No throttle data found for {selectedDrivers[0]} lap {selectedLap}
          </Text>
        </View>
      );
    }
    ...
    const chartData = throttleData.map((point, index) => ({
      x: point.Distance,
      y: point.Throttle,
    }));

    // Calculate domains for better visualization
    const allThrottleValues = throttleData.map(point => point.Throttle);
    const minThrottle = Math.min(...allThrottleValues);
    const maxThrottle = Math.max(...allThrottleValues);
    const yDomain: [number, number] = [Math.max(0, minThrottle - 5), Math.min(100, maxThrottle + 5)];

    // Calculate X-axis domain
    const maxDistance = throttleData.length > 0 ? throttleData[throttleData.length - 1].Distance : 0;
    const minDistance = throttleData[0]?.Distance || 0;
      ...
      <View style={{ marginTop: 16 }}>
        <View style={{
          ...
          <Text style={{
            ...
            {selectedDrivers[0]}'s {selectedLap === 'fastest' ? 'Fastest Lap' : `Lap ${selectedLap}`} Throttle Input
          </Text>
