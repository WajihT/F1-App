...
const BrakeChart: React.FC<BrakeChartProps> = ({
  ...
  const loadBrakeData = async () => {
    ...
    try {
      ...
      const data = await fetchBrakeData(year, event, session, selectedDriver, lapNumber);
      setBrakeData(data);
    } catch (error) {
  ...
  const renderChart = () => {
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#9ca3af', fontSize: 16, marginBottom: 16, textAlign: 'center' }}>
            Click load to view brake input for selected driver
          </Text>
          <TouchableOpacity
            onPress={loadBrakeData}
            style={{
          ...
          >
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '600' }}>
              {loading ? 'Loading...' : 'Load Brake Chart'}
            </Text>
          </TouchableOpacity>
        </View>
      );
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#f87171', fontSize: 16 }}>
            No brake data found for {selectedDriver} lap {selectedLap}
          </Text>
        </View>
      );
    }
    ...
    const chartData = {
      labels: brakeData.filter((_, index) => index % Math.ceil(brakeData.length / 6) === 0)
        .map(point => `${Math.round(point.Distance)}m`),
      datasets: [
        {
          data: brakeData.map(point => point.Brake),
