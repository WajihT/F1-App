...
const DRSChart: React.FC<DRSChartProps> = ({
  ...
  const toggleDriverSelection = (driverCode: string) => {
    setSelectedDrivers(prev => {
      ...
      if (shouldLoadChart && drsData.length > 0) {
  ...
  const loadDRSData = async () => {
    ...
    try {
      ...
      const data = await fetchDRSData(year, event, session, selectedDrivers[0], lapNumber);
      setDrsData(data);
    } catch (error) {
  ...
  const renderChart = () => {
      ...
      return (
        <View style={{
          ...
          <Text style={{ color: '#f87171', fontSize: 16 }}>
            No DRS data found for {selectedDrivers[0]} lap {selectedLap}
          </Text>
        </View>
      );
    }
    ...
    const chartData = drsData.map((point, index) => ({
      x: point.Distance,
      y: point.DRS,
    }));

    // Calculate domains for better visualization
    const allDRSValues = drsData.map(point => point.DRS);
    const minDRS = Math.min(...allDRSValues);
    const maxDRS = Math.max(...allDRSValues);
    const yDomain: [number, number] = [Math.max(0, minDRS - 0.1), Math.min(1, maxDRS + 0.1)];

    // Calculate X-axis domain
    const maxDistance = drsData.length > 0 ? drsData[drsData.length - 1].Distance : 0;
    const minDistance = drsData[0]?.Distance || 0;
      ...
      <View style={{ marginTop: 16 }}>
        <View style={{
          ...
          <Text style={{
            ...
            {selectedDrivers[0]}'s {selectedLap === 'fastest' ? 'Fastest Lap' : `Lap ${selectedLap}`} DRS Usage
          </Text>
